...
export class AuthService {
  ...
  async deleteAccount(): Promise<{ success: boolean; reason?: string }> {
    try {
      ...
      // 1. Soft delete: Marquer l'utilisateur comme supprimé dans Firestore
      // Cela permet aux administrateurs de voir que le compte a été supprimé
      try {
  ...
  async deleteClientAccount(clientId: string): Promise<{ success: boolean; reason?: string }> {
    try {
      // Soft delete: Marquer le client comme inactif au lieu de le supprimer
      // Cela garde l'historique pour l'administrateur
      const clientRef = doc(db, COLLECTIONS.clients, clientId);
      await setDoc(clientRef, {
        isActive: false,
        invitationStatus: 'declined', // Ou un statut spécifique "deleted" si dispo, mais 'declined' désactive l'accès
        deletedAt: Timestamp.now(),
        isDeleted: true
      }, { merge: true });

      // Si un compte utilisateur Firebase est lié, on le soft-delete aussi
      const clientData = await clientService.getClientById(clientId);
      if (clientData?.userId) {
        try {
          const userRef = doc(db, COLLECTIONS.users, clientData.userId);
          await setDoc(userRef, {
            isDeleted: true,
            deletedAt: Timestamp.now(),
            isBlocked: true // Bloquer l'accès explicitement
          }, { merge: true });
        } catch (uErr) {
          console.error('Erreur soft delete linked user:', uErr);
        }
      }
