...
export default function StepFeedbackList({ feedbacks, currentUserId }: StepFeedbackListProps) {
  ...
  const handleDelete = (item: VoiceNoteFeedback) => {
    const messageType = item.type === 'text' ? 'ce message' : 'cette note vocale';
    Alert.alert(
      "Supprimer le message",
      `Êtes-vous sûr de vouloir supprimer ${messageType} ? Cette action est irréversible.`,
      [
        { text: "Annuler", style: "cancel" },
        {
          ...
          onPress: async () => {
            try {
              if (item.id) {
                await feedbackService.deleteFeedback(item.chantierId, item.id);
              }
            } catch (err) {
